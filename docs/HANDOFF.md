...
## 最終更新
2026-09-16 マイナス金額（返金項目）取込バグ修正

## 2026-09-16 バグ修正：マイナス金額（返金項目）が内訳に取り込まれない

- **症状**：Excel取込で、マイナス金額（返金項目）が配信本文の内訳に取り込まれなかった。
  合計金額はExcelの合計セルをそのまま取得しているため正しいが、内訳から返金が欠落し、
  内訳の合算と表示合計が合わない状態になっていた。
  - 実例：加藤様（F24017）10月分の「9月分一部返金 -1,430円」「北入教材費返金 -11,000円」が内訳に出ていなかった。
- **原因**：`_extractSiblingBlock` の取込条件が `amount > 0` になっており、マイナス金額を弾いていた。
- **修正**：条件を `amount !== 0` に変更（`src/gas/コード.js`）。
  プラス・マイナス（返金）の両方を取り込み、ゼロ（空欄セル）は引き続き除外する。
  下流の `_generateBillingBody`（本文生成）・BillingItems生成側は金額フィルタを持たないため変更不要。
  表示は `toLocaleString()` のまま（マイナスは "-1,430円" と表示。記号変換なし＝Excel踏襲）。
- **動作確認**：加藤様のケースで返金2項目が内訳に反映され、配信も正常であることを確認済み。
- **教訓**：金額判定でマイナス（返金）を考慮すること。今後、金額フィルタを書く時は
  `amount !== 0` を基本とする（`> 0` はマイナスを取りこぼす）。
...
## 2026-06-20 達成：初回本配信成功

### プロジェクトの本丸が稼働
6月分（260626）の引き落とし通知を、41家族全員にLINE配信。

**Word差し込み印刷とメール配信の業務から、ふくちさんは完全に解放されました。**

### 当日のフロー
1. Phase 3-4：本配信機能の実装
   - executeRealBilling（全家族）
   - executeRealBillingSingle（個別）
   - preCheckRealBilling（事前チェック）
   - _updateBillingStatus（ステータス更新）
2. 取込実行UI改善：取込完了時に配信月プルダウンを自動更新
3. 6月分Excelの取込
4. データミス発見＆修正（同一家族の設定誤り3行）
5. 全家族テスト配信（41/41 成功）
6. 本配信実行（41/41 成功、エラー0）

### 安全策（実装した本配信の防御）
- 事前チェックダイアログで「対象家族数・送信予定・各種スキップ数」を表示
- 多重確認（confirm + prompt 入力）
- 既に配信済みのレコードはスキップ
- LINE_USER_ID 未登録の家族はスキップ
- 配信成功時：ステータス「配信済」と配信日時を記録
- 配信失敗時：ステータス「エラー」とエラー詳細を記録

### 次回（7月分配信）の手順
1. 既存業務通り、Excelに7月分シート（260726等）を作成
2. billing-line管理画面で取込（コピペ）
3. プレビューで41家族の本文を確認
4. 全家族テスト配信（ふくちさんに）
5. 本配信実行
...
## 2026-06-06 追加：未紐付けメッセージ画面の運用改善

### 1. 安定化対応

長期間使い続けると「読み込み中…」のまま固まる問題を解消：
- `getUnlinkedMessages` を最新300件スキャン・結果30件上限に
- メッセージ本文200文字制限、その他文字列も長さ制限
- try/catch で全体を保護（エラー時も必ず空配列を返す）

### 2. LINE_USER_ID コピー機能追加

未紐付けメッセージ画面（unlinked.html と admin.html 内のセクション）の
各カードに「📋 ID コピー」ボタンを追加：
- クリックでクリップボードにフルIDがコピーされる
- 生徒のLINE_USER_IDをマイ活Studentsシートに登録する運用で使用

### 運用ルール：生徒メッセージの処理フロー

1. 未紐付けメッセージ画面で「📋 ID コピー」
2. マイ活のStudentsシートに貼り付け
3. billing-line側で該当メッセージを「全て無視」で処理

## 2026-08-14 追加：GASのWebアプリ設定「アクセスできるユーザー」に注意

### 症状
- 未紐付けメッセージ画面に新着メッセージが反映されない
- スプレッドシートにも新しい行が追加されない
- ふくちさん自身は管理画面を開ける（自分だからアクセスできる）
- 実際には保護者・生徒からのLINEは届いている

### 原因
billing-line GAS の Webアプリ設定「アクセスできるユーザー」が
「自分だけ」になっていると、マイ活AWSからの Webhook 転送 POST が
実行権限エラーで弾かれ、IncomingMessages に記録されない。

### 対処
GASエディタ →「デプロイを管理」→ 鉛筆 → 
「アクセスできるユーザー」を「全員」に変更 → デプロイ

### 過去事例
2026-08-14：マイ活のAWS移行に伴い一時的に「自分だけ」に変えていた
設定を「全員」に戻すことで解決（8/4以降の受信データが記録されない問題）

### 恒久ルール
- billing-line GAS の Webアプリ設定「アクセスできるユーザー：全員」は必須
- 変更してはいけない
- appsscript.json 側でも `access: ANYONE_ANONYMOUS` を維持
